...
//
// @id = ch.banana.report.invoice.style02.js
// @api = 1.0
// @pubdate = 2016-05-06
// @publisher = Banana.ch SA
// @description = Style 2: address on the left, 3 colors
// @description.it = Stile 2: indirizzo sulla sinistra, 3 colori
// @description.de = Stil 2: Adresse links ausgedruckt, 3 Farben
// @description.fr = Style 2: adresse à gauche, 3 couleurs
// @description.en = Style 2: address on the left, 3 colors
// @task = report.invoice
...
function printInvoiceBase(jsonInvoice, repDocObj, repStyleObj, param) {
  ...
  **********************/
  ...
  tableRow.addCell(texts.invoice + " #", "", 1);
  ...
  tableRow = infoTable.addRow();
  tableRow.addCell(texts.payment_terms, "", 1);
  var payment_term = '';
  if (invoiceObj.billing_info.payment_term)
    payment_term = invoiceObj.billing_info.payment_term;
  if (payment_term.length <= 0 && param.payment_details_row_1.length > 0) {
    payment_term += param.payment_details_row_1;
  }
  tableRow.addCell(payment_term, "", 1);
...
function printInvoiceDetails(invoiceObj, repDocObj, param, texts, rowNumber) {
  ...
  tableRow.addCell(texts.payment_terms, "", 1);
  if (invoiceObj.billing_info.payment_term)
    tableRow.addCell(invoiceObj.billing_info.payment_term, "", 1);
  else
    tableRow.addCell("", "", 1);
